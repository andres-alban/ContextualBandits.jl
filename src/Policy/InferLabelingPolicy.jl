...
"""
    ...
    InferLabelingPolicy(subpolicy,selector,schedule,labeling0=trues((subpolicy.model.Wn+1)*subpolicy.model.m),sigma0=1e6,psi=1,D=fill(Inf,subpolicy.model.Wn,subpolicy.model.Wn),z_alpha=2,c=4)
...
mutable struct InferLabelingPolicy{T<:PolicyLinear, S<:LabelingSelector} <: Policy
    ...
    function InferLabelingPolicy(subpolicy::T,selector::S,schedule,labeling0=trues((subpolicy.model.Wn+1)*subpolicy.model.m),sigma0=1e6,psi=1,D=fill(Inf,subpolicy.model.Wn,subpolicy.model.Wn),z_alpha=2,c=4) where {T<:PolicyLinear,S<:LabelingSelector}
...
function initialize!(policy::InferLabelingPolicy,W,X,Y)
    ...
    theta, Sigma = default_prior_linear(policy.subpolicy.model.Wn, policy.subpolicy.model.m,
        policy.sigma0, policy.psi, policy.D, policy.subpolicy.model.labeling)
    robustify_prior_linear!(theta, Sigma, policy.subpolicy.model.Wn, policy.subpolicy.model.m, policy.subpolicy.model.labeling, policy.z_alpha, policy.c)
