...
This method is equivalent to 2^128 calls to the rand() function. An independent 
RNG can be spawned from the parent Xoshiro by first cloning the parent RNG, then
jumping its state to 2^128 places later in the sequence. Using this process, you
can generate 2^128 non-overlapping subsequences for parallel computations.

Source: https://discourse.julialang.org/t/how-to-spawn-child-rngs-with-random/102122/3?u=aal
"""
...
"""
    jump!(rng::AbstractRNG)

Reseed the rng using an integer generated by the rng. Workaround for the jump!
method for Xoshiro to generated parallel random numbers with other rngs.
"""
function jump!(rng::AbstractRNG)
    Random.seed!(rng, abs(rand(rng, Int)))
end